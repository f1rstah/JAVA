...
*/
        ...
        calculateRangeButton.addActionListener(e -> {
            try {
                ...
                Thread rangeCalculationThread1 = new Thread(() -> {
                    double currentH = h1;
                    while (currentH <= h2) {
                        double integral = integralTrapec(start, end, currentH, a1 -> Math.sin((Double) a1));
                        double finalCurrentH = currentH;
                        SwingUtilities.invokeLater(() -> addToTable(start, end, finalCurrentH, integral));
                        try {
                            Thread.sleep(0); // Задержка в одну секунду
                        } catch (InterruptedException ex) {
                            ex.printStackTrace();
                        }
                        currentH += 0.1; // или другой шаг, если требуется
                    }
                });

                rangeCalculationThread1.start();
